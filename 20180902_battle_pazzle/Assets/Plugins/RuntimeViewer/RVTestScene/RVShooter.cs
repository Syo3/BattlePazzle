...
public class RVShooter : MonoBehaviour
{
    ...
    List<RVCountdown> list_class = new List<RVCountdown>();
    List<int> list_int = new List<int>();

    void Start()
    {
        point = this.transform.Find("point");
        Transform b = this.transform.parent.Find("b");
        bullet = b.transform.Find("bullet");
        bulletExploder = b.transform.Find("bulletExploder");
        barPoint = this.transform.Find("barPoint");
        bullet.gameObject.SetActive(false);
        startDir = this.transform.forward;

        moveCD = new RVCountdown(GetStopMoveTime(), OnEnd_StopMove,OnUpdate_StopMove);
        moveCD.Start(true);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            list_class.Add(new RVCountdown(i*3));
            list_int.Add(i*3);
        }
    }
    ...
    void OnEnd_Move()
    {
        moveCD = new RVCountdown(GetStopMoveTime(), OnEnd_StopMove, OnUpdate_StopMove,0.9f, OnPer_StopMove);
        moveCD.Start(true);
    }
    ...
    void OnEnd_StopMove()
    {
        moveCD = new RVCountdown(GetMoveTime(), OnEnd_Move, OnUpdate_Move);
        moveCD.Start(true);
