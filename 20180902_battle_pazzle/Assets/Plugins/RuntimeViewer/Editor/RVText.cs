...
public class RVVisibility
{
    ...
    /// <summary>
    /// この値の親クラス,CollectionItemのばいはParentDataはそのCollection
    /// </summary>
    ...
    public void FieldInfoSetValue(object parent, object newItemValue)
    {
        ...
        if (isStruct == true)
        {
            ...
            //Debug.LogError(fieldInfo.GetValue(parent));

            //TypedReference tr = TypedReference.MakeTypedReference(parent, new FieldInfo[] { fieldInfo });
            //fieldInfo.SetValueDirect(tr, newItemValue);
        }
        else
        {
            fieldInfo.SetValue(parent, newItemValue);
